import { isStaticMode, staticFetchAPI, staticPostAPI } from './static-data.js';

// localhost → Deno API | github.io → static JSON | else → Web Daemon
...
async function fetchAPI(endpoint) {
    try {
        if (USE_STATIC) {
            return await staticFetchAPI(endpoint, currentProject);
        }
...
async function postAPI(endpoint, body) {
    try {
        if (USE_STATIC) {
            return await staticPostAPI(endpoint, body, currentProject);
        }
        const response = await fetch(`${API_BASE}${endpoint}`, {
            method: 'POST',
